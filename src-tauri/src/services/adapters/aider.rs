...
impl AgentAdapter for AiderAdapter {
    ...
    fn available_models(&self) -> Vec<ModelOption> {
        vec![
            ...
            ModelOption { id: "gemini/gemini-2.5-pro".into(), label: "Gemini 2.5 Pro".into() },
            ModelOption { id: "deepseek/deepseek-chat".into(), label: "DeepSeek Chat".into() },
        ]
    }

    async fn detect(&self) -> AgentDetection {
        let path = check_binary_in_path("aider").await;
        AgentDetection {
    ...
    async fn run(&self, params: AgentRunParams<'_>) -> Result<String, String> {
        ...
        cmd.arg("--message").arg(params.user_prompt)
            .arg("--no-git")
            .arg("--yes");
        ...
        let output = child.wait_with_output().await
            .map_err(|e| format!("Failed to wait on aider: {}", e))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(format!("Aider error ({}): {}", output.status, stderr));
        }
