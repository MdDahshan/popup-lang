...
impl AgentAdapter for GeminiAdapter {
    ...
    fn available_models(&self) -> Vec<ModelOption> {
        vec![
            ...
            ModelOption { id: "gemini-2.5-pro".into(), label: "Gemini 2.5 Pro".into() },
            ModelOption { id: "gemini-2.5-flash".into(), label: "Gemini 2.5 Flash".into() },
        ]
    }

    async fn detect(&self) -> AgentDetection {
        let path = check_binary_in_path("gemini").await;
        AgentDetection {
    ...
    async fn run(&self, params: AgentRunParams<'_>) -> Result<String, String> {
        ...
        let output = child.wait_with_output().await
            .map_err(|e| format!("Failed to wait on gemini: {}", e))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(format!("Gemini CLI error ({}): {}", output.status, stderr));
        }
